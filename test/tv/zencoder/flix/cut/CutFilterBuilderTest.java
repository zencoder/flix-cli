...
public class CutFilterBuilderTest {
   private BuilderTestHelper btHelper;
	...
	clHelper.setArgs(new String[] {"-cutstart", "1", "-cutstop", "4"});
	
	
	FlixBuilder[] builders = {new CutStartFilterBuilder(), new CutStopFilterBuilder()};
	...
	Filter filter = BuilderCache.getInstance().getCutFilterBuilder(btHelper.getFlix()).getFilter();
	
	try {
	    // Check the values that should have come from the child builders
	    assertEquals(new Double(1),  new Double(filter.getParam(flixengine2_internalConstants.FE2_CUT_START_SEC)));
	    assertEquals(new Double(4),  new Double(filter.getParam(flixengine2_internalConstants.FE2_CUT_STOP_SEC)));
